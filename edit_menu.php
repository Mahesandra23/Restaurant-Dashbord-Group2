...
if (isset($_GET['id'])) {
    ...
    if (isset($_POST['submit'])) {
        ...
        $sql = "UPDATE menu SET jenis= ?, nama_menu = ?, harga = ?, deskripsi = ? WHERE id_menu = ?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("ssssi", $jenis, $nama_menu, $harga, $deskripsi, $menu_id);
...
<nav class="navbar navbar-expand-lg navbar-light bg-white fixed-top shadow-bottom">
    ...
    <div class="container mx-auto login-signup editla">
        <h4 class="text-center">Admin Dashboard</h4>
        <h3 class="text-center">Edit Menu</h3>
        <form method="POST" action="">
            <div class="form-group mb-3">
                <label for="nama_menu">Menu Name:</label>
                <input type="text" class="form-control" name="nama_menu" value="<?= $menu_data['nama_menu'] ?>" required>
            </div>
            <div class="form-group mb-3">
                <label for="harga">Price:</label>
                <input type="number" class="form-control" name="harga" value="<?= $menu_data['harga'] ?>" required>
            </div>
            <div class="form-group mb-3">
            <label for="jenis">Jenis:</label>
            <select class="form-control-option" name="jenis" required>
                <?php
                $jenis_options = ["Food", "Dessert", "Drink"]; 
                foreach ($jenis_options as $option) {
                    $selected = ($option == $menu_data['jenis']) ? 'selected' : '';
            ...
            <div class="form-group mb-3">
                ...
                <textarea class="form-control" name="deskripsi" required><?= $menu_data['deskripsi'] ?></textarea>
            </div>
            <button type="submit" class="btn btn-primary mb-3" name="submit">Edit Menu</button>
        </form>
        <a href="admin.php" class="btn btn-secondary">Back to Menu List</a>
        </div>
    </div>
