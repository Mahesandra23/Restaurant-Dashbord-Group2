...
if (isset($_SESSION['user_id'])) {
    $loginSuccess = true;
    echo "<script>alert('Anda sedang login, mohon logout terlebih dahulu untuk membuat akun.'); window.location.href='login.php';</script>";
    exit();
}
$adminLoggedin = false;
...
if (isset($_POST['order'])) {
    ...
    if (isset($_SESSION['ordered_menu_ids'][$menuId])) {
        $_SESSION['ordered_menu_ids'][$menuId]++;
    } else {
        $_SESSION['ordered_menu_ids'][$menuId] = 1;
    }
}
...
if (isset($_POST['cancel'])) {
    ...
    if (isset($_SESSION['ordered_menu_ids'][$menuId])) {
        unset($_SESSION['ordered_menu_ids'][$menuId]);
    }
}

if (isset($_POST['confirm'])) {
    $_SESSION['ordered_menu_ids'] = array();

    echo "<script>alert('Order berhasil dikonfirmasi. Terima kasih!');</script>";
}
?>
...
<nav class="navbar navbar-expand-lg navbar-light bg-white fixed-top shadow-bottom">
    ...
    <div class="container order menu-list">
        <h1>Order Summary</h1>
        <div class="table-responsive">
            <table class="table">
                ...
                <tbody>
                    ...
                    $orderedMenuIds = isset($_SESSION['ordered_menu_ids']) ? $_SESSION['ordered_menu_ids'] : array();
                    if (!empty($orderedMenuIds)) {
                        $orderedMenuIdsString = implode(',', array_keys($orderedMenuIds));
                        $sql = "SELECT * FROM menu WHERE id_menu IN ($orderedMenuIdsString)";
                        $result = $conn->query($sql);

                        if ($result->num_rows > 0) {
                            while ($row = $result->fetch_assoc()) {
                                $menuId = $row['id_menu'];
                                $quantity = $orderedMenuIds[$menuId];
                                $price = $row['harga'];
                                $quantity = $orderedMenuIds[$menuId];
                                ...
                                echo "<td><img src='storage/" . $row['menu_image'] . "' alt='Menu Image' width='100'></td>";
                                echo "<td>" . $quantity . "</td>";
                                echo "<td>
                            ...
                            echo "<tr><td colspan='7'>No menu data found</td></tr>";
                        }
                    } else {
                        echo "<tr><td colspan='7'>No menu ordered</td></tr>";
                    }
        ...
        <h2>Total Price: Rp. <?php echo number_format($totalPrice, 2); ?></h2>
        <form method="POST" action="order.php">
            <button type="submit" class="btn btn-primary" name="confirm">Confirm Order</button>
        </form>
    </div>
    </div>
